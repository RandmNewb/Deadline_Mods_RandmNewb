...
eris_nvg.doMenu = function(_plID, _context, _items)
	...
	for i, items in ipairs(_items) do
		...
		if itemObj then
			if eris_nvg.isInPlayerInventory(nil, plObj, itemObj) and itemObj:getFullType() == "eris_nvg.nvgoggles42" then
...
eris_nvg.init = function()
	...
	eris_inventoryBar.registerItem("eris_nvg.nvgoggles41", "uiBattery_batteryLevel", getTextOrNull("IGUI_invpanel_Remaining") or "Remaining: ");
	eris_inventoryTooltip.registerItem("eris_nvg.nvgoggles41", "uiBattery_batteryLevel", getTextOrNull("IGUI_invpanel_Remaining") or "Remaining: ");
	eris_nvg.initInvInfo();
end
